...
<body>
    ...
    <section class="container section-inc-2">
        <div class="texto-curso">
            <h3 class="h3-inicio" style="font-size: 50px;">Cursos cadastrados</h3>
        </div>
    </section>
    ...
    <section class="container section-inc-video">
        <div class="row">
            <div class="video-inicio col-md-12">
                <h2 class="h2-curso" style="margin-top: 20px;">Escolha qual curso voce deseja continuar:</h2>
                <div id="cursos-cadastrados-lista" class="row"
                    style="margin-top: 40px; display: flex; flex-wrap: wrap;"></div>
            </div>
        </div>
    </section>
    ...
    <div id="modal-remover-curso" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.6); 
justify-content:center; align-items:center; z-index:9999;">
        <div style="background:white; padding:20px; border-radius:8px; max-width:300px; text-align:center;">
            <p style="font-size: 20px;">Deseja realmente remover o cadastro do curso?</p>
            <button id="btn-sim-remover" class="btn btn-primary" style="margin-right:10px; width: 100px;">Sim</button>
            <button id="btn-nao-remover" class="btn btn-secondary" style="width: 100px;">Não</button>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const listaCursos = document.getElementById('cursos-cadastrados-lista');

            if (listaCursos) {
                if (cursosCadastrados.length > 0) {
                    let cursosHTML = '';
                    cursosCadastrados.forEach((curso, index) => {
                        cursosHTML += `
                            <div class="col-md-4" style="margin-bottom: 20px;">
                            <div class="curso">
                                <img src="../images/imgsemfundo2.png" alt="${curso.nome}" style="width: 100%; height: 150px; object-fit: cover; border-radius: 10px;">
                                <h3>${curso.nome}</h3>
                                <p>Você está cadastrado nesse curso.</p>
                                <a href="videoaula.html" class="btn btn-primary">Acessar Curso</a>
                                <button style="margin-top: 10px;" class="btn btn-danger" onclick="removerCurso(${index}, '${curso.nome}')">Remover Cadastro</button>
                            </div>
                            </div>
                        `;
                    });
                    ...
                    listaCursos.innerHTML = '<p style="width: 100%; text-align: center;">Você não está cadastrado em nenhum curso.</p>';
                }
            }

            let cursoIndex;
            let cursoNome;
            window.removerCurso = function (index, nomeCurso) {
                cursoIndex = index;
                cursoNome = nomeCurso;
                const modalRemoverCurso = document.getElementById('modal-remover-curso');
                modalRemoverCurso.style.display = 'flex';
            }

            document.getElementById('btn-sim-remover').addEventListener('click', () => {
                const cursosCadastrados = JSON.parse(localStorage.getItem('cursosCadastrados')) || [];
                cursosCadastrados.splice(cursoIndex, 1);
                ...
                if (cursosDisponiveis[cursoNome]) {
                    cursosDisponiveis[cursoNome] = false;
                }
                localStorage.setItem('cursosDisponiveis', JSON.stringify(cursosDisponiveis));

                const modalRemoverCurso = document.getElementById('modal-remover-curso');
                modalRemoverCurso.style.display = 'none';
                window.location.reload();
            });

            document.getElementById('btn-nao-remover').addEventListener('click', () => {
                const modalRemoverCurso = document.getElementById('modal-remover-curso');
                modalRemoverCurso.style.display = 'none';
            });
        });
