    ...
    <div class="perfil-container">
        ...
        <div class="cards-container">
            <?php if ($matriculas->num_rows > 0): ?>
                <?php while ($mat = $matriculas->fetch_assoc()): ?>
                    <div class="card card-curso" data-tipo="curso" data-id="<?= htmlspecialchars($mat['curso_id']) ?>">
                        ...
                        <a href="../../api/professor/videoaula.php?id=<?= htmlspecialchars($mat['curso_id']) ?>"
                            class="btn btn-curso">
            ...
            <?php else: ?>
                <div class="mensagem-vazia">
                    ...
                    <strong>Matricule-se em um curso</strong> para começar a aprender!
                </div>
            <?php endif; ?>
        </div>
            ...
            <div class="cards-container">
                <?php if ($certificados->num_rows > 0): ?>
                    <?php while ($cert = $certificados->fetch_assoc()): ?>
                        <div class="card card-certificado" data-tipo="certificado"
                            ...
                            <a href="../../api/professor/gerar_certificado.php?curso_id=<?= htmlspecialchars($cert['curso_id']) ?>"
                ...
                <?php else: ?>
                    <div class="mensagem-vazia">
                        ...
                        <strong>Conclua seus cursos</strong> para emitir seus certificados!
                    </div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <script>
            ...
            inputFoto.addEventListener('change', async function () {
                ...
                try {
                    const res = await fetch('../../api/professor/alterar_foto.php', { method: 'POST', body: formData });
            ...
            function mostrarModalSucesso(msg) {
                ...
                modal.innerHTML = `<i class="fa fa-check-circle"></i> ${msg}`;
            ...
            function mostrarModalErro(msg) {
                ...
                modal.innerHTML = `<i class="fa fa-times-circle"></i> ${msg}`;
                document.body.appendChild(modal);
                setTimeout(() => modal.classList.add('ativo'), 10);
                setTimeout(() => { modal.classList.remove('ativo'); setTimeout(() => modal.remove(), 300); }, 3000);
            }

            // Duplo clique nos cards
                ...
                card.addEventListener('dblclick', function () {
                    ...
                    if (tipo === 'curso') {
                        ...
                        modalBody.innerHTML = `<h3>Curso: ${this.querySelector('h4').innerText}</h3>
                            ...
                            <a href="../../api/professor/videoaula.php?curso_id=${id}" class="btn" style="display:inline-block;margin-top:10px;">Acessar Curso</a>`;
                        ...
                        modalBody.innerHTML = `<h3>Certificado: ${this.querySelector('h4').innerText}</h3>
                            ...
                            <a href="../../api/professor/gerar_certificado.php?curso_id=${curso}" class="btn" style="display:inline-block;margin-top:10px;"><i class="fa fa-download"></i> Baixar Certificado</a>`;
                        modalBg.style.display = 'flex';
                    }
                });
            });
